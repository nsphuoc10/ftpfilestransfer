...
public class LocalFilesArrayAdapter extends ArrayAdapter<File>{


    private List<File> mListFilesTransfer;
    private Activity mActivity;
    private int mLayoutId;
    private LayoutInflater mInflater;

    public LocalFilesArrayAdapter(Context context, int resource, List<File> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        File fileTransfer = mListFilesTransfer.get(position);
        viewHolder.tvTitle.setText(fileTransfer.getName());
        viewHolder.tvDate.setText(Utilities.parseDateFromMiliSeconds(fileTransfer.lastModified()));
        if(fileTransfer.isDirectory()){
            ...
            viewHolder.tvSize.setText(Utilities.parseSize(fileTransfer.length()));
            viewHolder.ivIcon.setImageResource(Utilities.getIconViaExtension(mActivity,
                    Utilities.getMimeType(mActivity, fileTransfer)));
        }
        return convertView;
    }
